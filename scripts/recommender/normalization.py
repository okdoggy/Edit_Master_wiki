...
SLOT_PATTERNS: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    "where": {
        ...
        "place:mountain": ("산", "풍경", "하늘", "mountain", "landscape", "sky", "foreground"),
    ...
    "what": {
        "intent:portrait": ("인물", "사람", "사람 사진", "얼굴", "프로필", "portrait", "profile"),
        "intent:couple_portrait": ("커플", "couple"),
        "intent:selfie": ("셀카", "selfie"),
        "intent:group_photo": ("친구", "친구들이랑", "단체", "그룹", "여럿", "group photo", "group selfie", "friends"),
        ...
        "intent:action_motion": ("자전거", "러너", "달리는", "뛰는", "액션", "cyclist", "runner", "running", "sports", "action shot"),
        ...
        "intent:document_scan": ("문서", "계약서", "화이트보드", "글자", "스캔", "scan", "document", "whiteboard", "readable"),
        "intent:receipt_flatlay": ("영수증", "영수증샷", "포토덤프", "photo dump", "receipt", "receipt photo"),
        "intent:dating_profile": ("소개팅", "데이팅", "데이팅 앱", "dating profile", "meeting new people"),
    ...
    "how": {
        ...
        "mode:no_flash": ("플래시 없이", "플래시는 못", "플래시 금지", "no flash", "without flash", "flash off"),
        "light:backlight": ("역광", "backlight", "backlit", "back lit"),
        "light:rim_light": ("림라이트", "테두리 빛", "머리카락 빛", "머리카락 테두리", "hair glow", "rim light"),
        "light:harsh_light": ("강한 빛", "햇빛 너무", "빛이 너무", "쨍한 빛", "harsh light", "harsh sun"),
        ...
        "edit:perspective": ("사다리꼴", "원근", "기하", "skewed", "perspective", "straighten", "geometry", "upright"),
    },
    "issues": {
        "issue:underexposed_face": ("얼굴 어둡", "얼굴이 어둡", "얼굴이 어두", "얼굴은 어둡", "얼굴은 어두", "얼굴 한쪽", "칙칙", "face dark", "underexposed face", "shadow on face"),
        "issue:busy_background": ("배경 지저분", "배경이 지저분", "뒤 테이블", "사람 많", "산만", "busy background", "cluttered"),
        "issue:blown_highlights": ("하늘 날아", "하이라이트", "네온만 너무", "날아가", "날아갔", "날림", "너무 밝", "밝게 날아", "쨍", "쨍하게", "과노출", "하얗게", "빛이 터", "blown", "clipping", "overexposed"),
        "issue:hair_highlight_clipping": ("머리 위로 쨍", "머리 위 빛", "머리카락만 너무 밝", "머리카락 하이라이트", "머리 하이라이트", "테두리 빛", "림라이트가 너무", "rim light too strong", "hair highlight clipping"),
        ...
        "issue:edge_distortion": ("왜곡", "비율 이상", "다리 짧", "가장자리 얼굴", "늘어나", "distortion", "proportion", "edge face", "faces stretched", "wide angle face"),
        ...
        "issue:document_skew": ("사다리꼴", "휘고", "모서리", "skewed", "corners", "perspective"),
