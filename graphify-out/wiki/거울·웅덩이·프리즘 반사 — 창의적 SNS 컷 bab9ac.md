---
source_file: "raw/scenarios/reflection_mirror_puddle_prism.md"
type: "document"
community: "Trends & Recipes 3"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Trends_&_Recipes_3
---

# 거울·웅덩이·프리즘 반사 — 창의적 SNS 컷 [bab9ac]

## Connections
- [[0.5x 1x 2x]] - `USES_LENS` [EXTRACTED]
- [[adobe.comsmartphone-photography.html]] - `SUPPORTED_BY` [EXTRACTED]
- [[business.adobe.comcreative-trends-report.html]] - `SUPPORTED_BY` [EXTRACTED]
- [[creative]] - `HAS_SCENE_TAG` [EXTRACTED]
- [[creative reflection]] - `SUPPORTS_STYLE` [EXTRACTED]
- [[helpx.adobe.commasking-mobile-ios.html]] - `SUPPORTED_BY` [EXTRACTED]
- [[light leak]] - `TARGETS_CONCEPT` [EXTRACTED]
- [[low angle]] - `USES_CONCEPT` [EXTRACTED]
- [[mirror]] - `HAS_SCENE_TAG` [EXTRACTED]
- [[mirror prism puddle]] - `USES_CONCEPT` [EXTRACTED]
- [[person or city]] - `HAS_SUBJECT` [EXTRACTED]
- [[playful visual hook]] - `FOLLOWS_TREND` [EXTRACTED]
- [[prism 02434e]] - `HAS_SCENE_TAG` [EXTRACTED]
- [[prism 2fcae6]] - `USES_CONCEPT` [EXTRACTED]
- [[puddle a6f2d6]] - `HAS_SCENE_TAG` [EXTRACTED]
- [[puddle e05a3f]] - `TARGETS_CONCEPT` [EXTRACTED]
- [[reflection c937e1]] - `HAS_SCENE_TAG` [EXTRACTED]
- [[reflection ef584d]] - `USES_CONCEPT` [EXTRACTED]
- [[samsung.comcreative-photography-tricks-thatll-get-you-all-the-likes]] - `SUPPORTED_BY` [EXTRACTED]
- [[sns f490e5]] - `HAS_SCENE_TAG` [EXTRACTED]
- [[subject]] - `TARGETS_CONCEPT` [EXTRACTED]
- [[symmetry]] - `USES_CONCEPT` [EXTRACTED]
- [[urban or nature]] - `IN_ENVIRONMENT` [EXTRACTED]
- [[visual interest]] - `TARGETS_CONCEPT` [EXTRACTED]
- [[거울·웅덩이·프리즘 반사 — 창의적 SNS 컷 a4e1cb]] - `DESCRIBES_SCENARIO` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Trends_&_Recipes_3