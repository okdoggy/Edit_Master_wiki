---
source_file: "raw/scenarios/beach_backlit_portrait.md"
type: "document"
community: "Trends & Recipes 2"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Trends_&_Recipes_2
---

# face

## Connections
- [[2x portrait e06427]] - `FLATTERS` [EXTRACTED]
- [[subject mask]] - `PROTECTS` [EXTRACTED]
- [[눈 오는 날 인물 — 깨끗한 겨울하이키 스타일 8cdb47]] - `TARGETS_CONCEPT` [EXTRACTED]
- [[한낮 해변 인물 — 강한 그림자와 날아간 하늘 잡기 8e8f2c]] - `TARGETS_CONCEPT` [EXTRACTED]
- [[해변 역광 인물 — 여름휴양지 SNS 스타일 7fa63d]] - `TARGETS_CONCEPT` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Trends_&_Recipes_2