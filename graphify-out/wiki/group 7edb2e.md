---
source_file: "raw/scenarios/group_best_face_gathering.md"
type: "document"
community: "Trends & Recipes 3"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Trends_&_Recipes_3
---

# group [7edb2e]

## Connections
- [[단체 모임 그룹샷 — 눈 감음과 표정 실패를 줄이는 베스트컷 루프 24988e]] - `HAS_SUBJECT` [EXTRACTED]
- [[단체 여행 셀피그룹샷 — 얼굴 왜곡 줄이기 53a815]] - `HAS_SUBJECT` [EXTRACTED]
- [[실내 생일파티·모임 단체사진 — 어두운 실내와 움직임 1a52fb]] - `HAS_SUBJECT` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Trends_&_Recipes_3