---
source_file: "raw/recommendation/natural_answer_renderer_guidelines.md"
type: "document"
community: "Evidence Sources 2"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Evidence_Sources_2
---

# Natural answer renderer guidelines — graph result to user-facing photo coaching

## Connections
- [[answer_renderer_contract]] - `HAS_CONTENT_TYPE` [EXTRACTED]
- [[recommendation]] - `IN_CATEGORY` [EXTRACTED]
- [[그래프 노드 → 자연어 변환 예시]] - `HAS_SECTION` [EXTRACTED]
- [[기본 출력 템플릿]] - `HAS_SECTION` [EXTRACTED]
- [[다시 찍는다면]] - `HAS_SECTION` [EXTRACTED]
- [[답변 원칙]] - `HAS_SECTION` [EXTRACTED]
- [[목적 8477e8]] - `HAS_SECTION` [EXTRACTED]
- [[스타일 선택]] - `HAS_SECTION` [EXTRACTED]
- [[지금 사진을 보정한다면]] - `HAS_SECTION` [EXTRACTED]
- [[한 줄 진단]] - `HAS_SECTION` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Evidence_Sources_2